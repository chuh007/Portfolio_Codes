...
namespace _Work.CHUH.Code.Tree.UI
{
    ...
    public class TreeUIController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            scrollRect.verticalNormalizedPosition = 1f; // 세로 트리의 위쪽에서 시작
            _isInitialized = true;
        ...
        {
            _levelNodeIdList.Clear();
            var allNodes = TreeDataManager.Instance.AllNodes;

            if (TryUseConfiguredLayout(allNodes)) return;
        ...
        private bool TryUseConfiguredLayout(IReadOnlyDictionary<string, TreeNodeDataSO> allNodes)
        {
            if (allNodes.Count == 0 || allNodes.Values.Any(node => node.uiRow < 0))
                return false;

            foreach (TreeNodeDataSO parent in allNodes.Values)
            {
                if (parent.childNodes.Any(child => child == null || child.uiRow <= parent.uiRow))
                {
                    Debug.LogWarning("[TreeUIController] UI 행 설정이 연결 방향과 맞지 않아 자동 배치를 사용합니다.", this);
                    return false;
                }
            }

            foreach (IGrouping<int, TreeNodeDataSO> row in allNodes.Values
                         .GroupBy(node => node.uiRow)
                         .OrderBy(group => group.Key))
            {
                _levelNodeIdList.Add(row
                    .OrderBy(node => node.uiOrder)
        ...
        private void LinkAllNode()
        {
            ...
            {
                foreach (var nodeData in nodeObject.NodeData.childNodes)
                {
